...
@WebServlet(name = "LoginHistoryListCtl", urlPatterns = { "/ctl/LoginHistoryListCtl" })
public class LoginHistoryListCtl extends BaseCtl {

	@Override
	protected void preload(HttpServletRequest request, HttpServletResponse response)
			throws IOException, ServletException {
		LoginModel nameModel = new LoginModel();
		try {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<LoginBean> next = model.search(bean, pageNo + 1, pageSize);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op)) {
				...
				pageNo = 1;
				LoginBean deletebean = new LoginBean();
	...
	protected String getView() {
		return ORSView.LOGIN_HISTORY_LIST_VIEW;
	}
}